  ...
  <div class="bg-gray-200 rounded-2xl p-6 sm:p-8 w-full max-w-3xl space-y-6 shadow-lg overflow-hidden">
    ...
    <div class="flex justify-between items-center mb-6 flex-wrap gap-4">
      ...
      <button @click="showModal = true"
        class="bg-orange-600 hover:bg-orange-700 shadow-md font-bold text-white px-4 sm:px-6 py-2 sm:py-3 rounded-full text-sm sm:text-base flex items-center space-x-2">
    ...
    <!-- Appointment list -->
    <div v-if="appointments.length" class="space-y-4 max-h-[400px] overflow-y-auto pr-2">
      <div v-for="appointment in appointments" :key="appointment.id"
        class="bg-white p-4 sm:p-6 rounded-xl shadow hover:border-blue border hover:border-slate-400 text-base sm:text-lg relative">
        <!-- Vehicle Info -->
        ...
        <div class="absolute top-4 right-1">
          <button @click="toggleMenu(appointment.id)" class="text-2xl px-2 text-gray-600 hover:text-black">
            <EllipsisVertical />
          </button>
          <div v-if="showMenuId === appointment.id"
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-4">
      <div class="bg-white p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-lg space-y-6">
        ...
        <input v-model="appointmentDate" type="datetime-local"
          class="border border-gray-300 p-3 sm:p-4 w-full rounded-lg text-base sm:text-lg" />

        <textarea v-model="note" placeholder="Notat" rows="4"
          class="border border-gray-300 p-3 sm:p-4 w-full rounded-lg text-base sm:text-lg"></textarea>

        <div class="flex justify-end space-x-4 pt-2">
          <button @click="showModal = false" class="text-gray-600 text-base px-4 py-2">Avbryt</button>
          <button @click="createAppointment"
            class="bg-orange-600 hover:bg-orange-700 text-white px-6 py-2 rounded-lg text-base">
            Lagre
          </button>
        </div>
      </div>
