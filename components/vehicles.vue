  ...
  <div class="bg-gray-200 rounded-2xl p-8 w-full max-w-3xl space-y-6 shadow-lg">
    <div class="flex justify-between items-center mb-6">
      ...
      <button
        ...
        class="bg-orange-600  hover:bg-orange-700 font-bold shadow-md text-white px-6 py-3 rounded-full text-base flex items-center space-x-2"
      >
    ...
    <div v-if="vehicleStore.vehicles.length" class="space-y-4">
      <div
        v-for="vehicle in vehicleStore.vehicles"
        :key="vehicle.id"
        class="relative flex items-center bg-white rounded-xl p-6 shadow-md border hover:border-slate-400 transition-all text-lg"
      >
        <Car class="w-8 h-8 text-gray-700 mr-6" />
        <div class="flex-1">
          <div class="font-semibold text-xl">{{ vehicle.name }}</div>
          <div class="text-base mt-1">Reg: {{ vehicle.license_plate }}</div>
          <div class="text-base mt-1">Km: {{ vehicle.kilometers?.toLocaleString() ?? 0 }}</div>
        </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div class="bg-white p-8 rounded-xl shadow-2xl w-full max-w-lg space-y-6">
        ...
        <input
          ...
          placeholder="Skilt"
          class="border border-gray-300 p-4 w-full rounded-lg text-lg"
        />
        ...
        <div class="flex justify-end space-x-4 pt-2">
          ...
          <button @click="addVehicle" class="bg-orange-600 hover:bg-orange-700 text-white px-6 py-2 rounded-lg text-lg">Lagre</button>
        </div>
      </div>
    </div>
  </div>
